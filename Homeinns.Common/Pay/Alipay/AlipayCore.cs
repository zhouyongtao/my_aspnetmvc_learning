...
namespace Homeinns.Common.Pay
{
    ...
    public class AlipayCore
    {
        ...
        public static string CreateLinkStringUrlencode(Dictionary<string, string> dicArray, Encoding code)
        {
            ...
            foreach (KeyValuePair<string, string> temp in dicArray)
            {
                prestr.Append(String.Format("{0}={1}&", temp.Key, HttpUtility.UrlEncode(temp.Value, code)));
            }
            //去掉最後一個&字符
            int nLen = prestr.Length;
            prestr.Remove(nLen - 1, 1);
        ...
        {
            string logsDir = AppDomain.CurrentDomain.BaseDirectory + "\\VerifyLog";
            if (!Directory.Exists(logsDir))
            {
                Directory.CreateDirectory(logsDir);
            }
            using (StreamWriter fs = new StreamWriter(String.Format("{0}\\{1:yyyy-MM-dd}.log", logsDir, DateTime.Today), true, Encoding.Default))
            {
                fs.WriteLine(string.Format("VerifyLog Time:{0}-------\r\n :{1}", DateTime.Now, sWord));
                fs.Flush();
                fs.Close();
            }
        }
