...
using System.Web;
using Homeinns.Common.Base.Regular;
...
namespace Homeinns.Common.Net
{
    ...
    public class HttpUtil
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static RemoteArddess getIPAndPortByHead()
        {
            ...
            RemoteArddess r = new RemoteArddess();
            HttpRequestMessageProperty a = propertites["httpRequest"] as HttpRequestMessageProperty;
            string b = Chk.IsNull(a.Headers["Client-IP"]);
            if (b == string.Empty || !RegexHelper.IsIP(b))
            {
                r.IP = endPoint.Address;
                r.Port = endPoint.Port.ToString();
            }
            else
            {
                string[] arr = b.Split(':');
        ...
        public static string HttpProxy(string url, string method = "GET", string data = null, string contentType = "application/x-www-form-urlencoded", string soapAction = null, int timeout = 8000)
        {
            ...
            {
                NRLog.ExceptionLog(String.Format("HttpProxy Exception url: {0} \r\n data: {1}", url, data), ex);
                return string.Empty;
            }
        }
        ...
        public static void HttpProxyAsync(string url, string method = "GET", string data = null, string contentType = "application/x-www-form-urlencoded", int timeout = 1000)
        {
            try
            {
                ...
                request.BeginGetResponse(AsyncResponseCallback =>
                {
                    ...
                    {
                        //调用失败
                        NRLog.ExceptionLog(result, new Exception());
                    }
                    else
                    {
            ...
            {
                NRLog.ExceptionLog(String.Format("HttpProxyAsync Exception url: {0} \r\n data: {1}", url, data), ex);
            }
        }
        #endregion
    }
