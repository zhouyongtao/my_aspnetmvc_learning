...
namespace Homeinns.Common.Net.Http
{
	...
	public class Range
	{
        private readonly static Regex _parseReg = new Regex(@"^([^\s]+)\s*(\d+)-(\d+)/(\d+)$", RegexOptions.IgnoreCase | RegexOptions.Singleline);
		/// <summary>
		/// 表示开始位置
		/// </summary>
		...
		{
			var m = _parseReg.Match(arg);
			if (!m.Success) throw new ArgumentException("无法分析指定的 Content-Range 标记 - " + arg);
